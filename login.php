...
if (mfa_pending_user_id() > 0) {
    header('Location: mfa_verify.php');
    exit;
}

$showError = isset($_GET['error']) && $_GET['error'] === 'invalid';
$showMfaSendError = isset($_GET['error']) && $_GET['error'] === 'mfa_send';
...
<body class="min-h-screen bg-gradient-to-br from-slate-100 via-blue-50 to-slate-100 text-slate-900 antialiased">
    <div class="min-h-screen flex flex-col lg:flex-row">
        ...
        <!-- Feature panel -->
        <section
    class="relative hidden lg:flex lg:w-1/2 xl:w-3/5 flex-col justify-between overflow-hidden bg-gradient-to-br from-emerald-900 via-teal-900 to-slate-900 bg-[size:200%_200%] animate-gradient-shift motion-reduce:animate-none px-12 py-14 xl:px-16"
>
            <div class="max-w-md">
        ...
        <main class="flex w-full flex-1 items-center justify-center px-6 py-12 sm:px-10 lg:w-1/2 xl:w-2/5">
            <div class="w-full max-w-sm animate-fade-slide-up motion-reduce:animate-none">
                ...
                <div id="login-view" class="transition-opacity duration-300 ease-in-out" aria-hidden="false">
                    ...
                    <?php if ($showMfaSendError): ?>
                        <div class="mb-6 flex items-start gap-2 rounded-lg border border-red-200 bg-red-50 px-4 py-3" role="alert">
                            <svg class="mt-0.5 h-4 w-4 shrink-0 text-red-500" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM9 9a1 1 0 112 0v4a1 1 0 11-2 0V9zm1-4a1 1 0 100 2 1 1 0 000-2z" clip-rule="evenodd"/></svg>
                            <p class="text-sm font-medium text-red-600">
                                <?= htmlspecialchars('Unable to send the verification code. Please contact the System Administrator or try again later.', ENT_QUOTES, 'UTF-8') ?>
                            </p>
                        </div>
                    <?php endif; ?>
                    ...
                    <form method="POST" action="auth.php" class="space-y-5">
                        ...
                        <button
                            type="submit"
                            class="w-full rounded-lg bg-emerald-700 px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-emerald-800 hover:shadow-md focus:outline-none focus:ring-2 focus:ring-emerald-600 focus:ring-offset-2 transition-all duration-200 ease-out hover:-translate-y-0.5 hover:scale-[1.02] hover:shadow-md active:translate-y-0 active:scale-100 motion-reduce:transition-none"
                        >
                            Sign in
                        </button>
                    </form>
